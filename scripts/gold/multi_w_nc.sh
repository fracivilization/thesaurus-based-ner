...
dir=`dirname $0`
OUTPUT_DIR=outputs/gold/multi_w_nc
mkdir -p ${OUTPUT_DIR}
pwd >> ${OUTPUT_DIR}/cout
...
for negative_ratio in ${negative_ratios[@]}; do
    ...
    MAKE="WITH_NEGATIVE_CATEGORIES=True MSMLC_STATIC_PN_RATIO_EQUIVALENCE=True MSMLC_NEGATIVE_RATIO_OVER_POSITIVE=${negative_ratio} make"
