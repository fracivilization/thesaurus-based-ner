...
for eval_step in ${eval_steps[@]}; do
    for early_stop_patience in ${early_stop_patiences[@]}; do
        echo "early_stop_patience: ${early_stop_patience}, eval_steps: ${eval_step}" >>${OUTPUT_DIR}/cout
        MAKE="TRAIN_BATCH_SIZE=8 EVAL_BATCH_SIZE=16 EVAL_STEPS=${eval_step} EARLY_STOPPING_PATIENCE=${early_stop_patience} EVAL_DATASET=${EVAL_DATASET} WITH_O=True FIRST_STAGE_CHUNKER=\"enumerated\" make"
