...
def multi_source_label_refine(
    args, hp_labels, combined_labels, pred_labels, pad_token_label_id, pred_logits=None
):
    ...
    if args.self_training_label_mode == "hard":
        if args.self_training_hp_label == 0:
            ...
            softmax = torch.nn.Softmax(dim=1)
            y = softmax(pred_logits.view(-1, pred_logits.shape[-1])).view(
                pred_logits.shape
            )
            ...
            pred_labels[_confidence < _threshold] = combined_labels[
                _confidence < _threshold
            ]
        else:
