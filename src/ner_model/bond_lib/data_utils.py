...
def remove_unused_columns(dataset: "datasets.Dataset", model):
    if isinstance(model, DataParallel):
        signature = inspect.signature(model.module.forward)
    else:
...
def convert_examples_to_features(
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        full_label_ids = []
        hp_label_ids = []
        for word, label, hp_label in zip(
            example.words, example.labels, example.hp_labels
        ):
            ...
            hp_label_ids.extend(
                [hp_label if hp_label is not None else pad_token_label_id]
        ...
        if ex_index < show_exnum:
            ...
            logger.info(
                "full_label_ids: %s", " ".join([str(x) for x in full_label_ids])
            )
        ...
        features.append(
            InputFeatures(
                ...
                label_ids=label_ids,
                full_label_ids=full_label_ids,
                hp_label_ids=hp_label_ids,
            )
        )
...
def load_and_cache_examples(args, tokenizer, labels, pad_token_label_id, mode):
    ...
    all_full_label_ids = torch.tensor(
        [f.full_label_ids for f in features], dtype=torch.long
    )
    all_hp_label_ids = torch.tensor(
        [f.hp_label_ids for f in features], dtype=torch.long
    )
