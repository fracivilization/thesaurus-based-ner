...
def evaluate(
    ...
    remove_unused_columns(eval_dataset, model)
    ...
    eval_dataloader = DataLoader(
        ...
        collate_fn=DataCollatorForTokenClassification(
            tokenizer,
            max_length=max(len(snt) for snt in eval_dataset["input_ids"]),
            padding="max_length",
        ),
    )
    # multi-gpu evaluate
    ...
    correct_preds, total_correct, total_preds = 0.0, 0.0, 0.0  # i variables
    for ground_truth_id, predicted_id in zip(out_id_list, preds_id_list):
        # We use the get chunks function defined above to get the true chunks
        # and the predicted chunks from true labels and predicted labels respectively
        lab_chunks = set(
            get_chunks(ground_truth_id, {tag: i for i, tag in enumerate(labels)})
        )
        lab_pred_chunks = set(
            get_chunks(predicted_id, {tag: i for i, tag in enumerate(labels)})
        )
        # remove fake cat
        lab_pred_chunks = {
            (l, s, e) for l, s, e in lab_pred_chunks if not l.startswith("fake_cat_")
        }
