...
class HAMNERConfig(TyperConfig):
    ...
    training_args: HydraAddaptedTrainingArguments = HydraAddaptedTrainingArguments(
        output_dir="."
    )
...
class HAMNERTyper(Typer):
    ...
    def load_model_input(
        ...
        all_starts = []
        all_ends = []
        for snt_id, (snt, starts, ends) in enumerate(
            zip(example["tokens"], example["starts"], example["ends"])
        ):
